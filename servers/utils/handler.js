...
function createHandler(options, benchmark) {
  ...
  return async function requestListener(req, res) {
    ...
    try {
      const [body, init] = await handle({
        ...
        body: () => {
          ...
          return new Promise(resolve => {
            let dataBody = '';
            req.on('data', chunk => (dataBody += chunk));
            req.on('end', () => resolve(dataBody));
          });
        },
        raw: req,
        context: { res },
      ...
      jsonBody.benchmark = await benchmark(startTime);
      res.writeHead(init.status, init.statusText, init.headers).end(JSON.stringify(jsonBody));
      jsonBody = null;
    } catch (err) {
      // The handler shouldnt throw errors.
      // If you wish to handle them differently, consider implementing your own request handler.
      console.error('Internal error occurred during request handling. ' + 'Please check your implementation.', err);
